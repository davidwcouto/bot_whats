...
function pesquisaValida(pesquisa) {
    ...
    return tiposPermitidos.some(tipo => pesquisa.includes(tipo));
}
...
Digite novamente sua consulta.`;
    ...
    try {
        const resposta = await axios.get("https://api.gestaoclick.com/produtos", {
            headers: {
        ...
        const encontrados = produtos.filter(item => {
            ...
            return nomeProduto.includes(pesquisa);
        });
...
Para atendimento digite 2️⃣`;
        ...
        let mensagem = `🔎 Encontrei estas opções para *${produto.toUpperCase()}*:\n\n`;

        encontradosLimitados.forEach((item, index) => {
            mensagem += `${index + 1}️⃣ *${item.nome}*\n`;
            mensagem += `💰 R$ ${formatarPreco(item.valor_venda)}\n`;
        ...
        console.error("Erro ao consultar GestãoClick:", erro.response?.data || erro.message);
